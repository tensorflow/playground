...
export type DataGenerator = {
  call: (numSamples: number, noise: number) => Example2D[],
  variant: 'data_generator',
};

export function isDataGenerator(x: any): x is DataGenerator {
  return (x as DataGenerator).variant === 'data_generator'
}
...
export function isDataLoader(x: any): x is DataLoader {
  return (x as DataLoader).variant === 'data_loader'
}
